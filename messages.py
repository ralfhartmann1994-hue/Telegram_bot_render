# =================
# messages.py (معدل مع دمج الرسائل وتحديث الترحيب)
# =================
import random
import time
...
SEARCHING = "🔍 جاري البحث عن شريك مناسب لك...\n⏰ قد يستغرق الأمر حتى 30 دقيقة\n💡 يمكنك إلغاء البحث في أي وقت!"
SEARCH_CANCELLED = "❌ تم إلغاء البحث بنجاح!\n🔍 يمكنك البحث مرة أخرى في أي وقت."
...
# رسائل ترحيبية عامة (معدلة)
START_MESSAGES = [
    ".... اهلا وثهلا بك في برنامذنا المتواضع حيث ثتجد هنا اثدقاء ممتاذين وفئات نقاش متعددة وثندويشة شاورما وكما تعلم ان الثقثقة في الثندويشة مهمة جدا لجودة اللوحة فلا يمكنك ان تنظر لجزء من اللوحة وتثألني وان اردنا ان نعرف ماذا في البراذيل يجب ان نعرف ماذا في المانيا...."
]

# رسائل رياضية (دمج FOOTBALL مع SPORTS)
SPORTS_WELCOME_MESSAGES = [
    "أهلاً بالمحنك الرياضي! رح تعرف تدافع عن فريقك؟",
    "⚽ يلا نشوف... ريال مدريد ولا برشلونة؟ (وإذا قلت برشلونة رح أحترمك أكتر 😂)",
...
# رسائل سياسية
POLITICS_WELCOME_MESSAGES = [
    "أهلاً بالمحنك السياسي زعيم غرفته!",
...
# رسائل دينية
RELIGION_WELCOME_MESSAGES = [
    "أهلاً بطالب العلم الروحاني الباحث عن الحكمة!",
...
# رسائل فلسفية
PHILOSOPHY_WELCOME_MESSAGES = [
    "أهلاً بالفيلسوف العميق الذي يبحث عن معنى الحياة!",
...
# رسائل اجتماعية
SOCIAL_WELCOME_MESSAGES = [
    "أهلاً بالشخص الاجتماعي الذي يحب اللقاءات الجديدة!",
...
def get_welcome_message(topic=None):
    """دالة للحصول على رسائل الترحيب العامة عند بداية البوت"""
    if topic == "START":
    ...
    t = topic.lower()
    if "رياض" in t or "⚽" in topic or "كرة" in t or "football" in t.lower() or "sport" in t:
        return random.choice(SPORTS_WELCOME_MESSAGES)
    elif "سياس" in t or "politics" in t.lower():
        return random.choice(POLITICS_WELCOME_MESSAGES)
    elif "دين" in t or "religion" in t.lower() or "إسلام" in t:
        return random.choice(RELIGION_WELCOME_MESSAGES)
    elif "فلسف" in t or "philosophy" in t.lower():
        return random.choice(PHILOSOPHY_WELCOME_MESSAGES)
    elif "تعارف" in t or "social" in t.lower() or "اجتماع" in t:
        return random.choice(SOCIAL_WELCOME_MESSAGES)
...
def get_category_welcome(category):
    """دالة للحصول على رسائل ترحيبية خاصة بكل فئة عند اختيار الموضوع"""
    if not category:
        return "مرحباً بك!"
    
    category = category.lower()
    if "رياض" in category or "sport" in category:
        return random.choice(SPORTS_WELCOME_MESSAGES)
    elif "سياس" in category or "politic" in category:
        return random.choice(POLITICS_WELCOME_MESSAGES)
    elif "دين" in category or "religion" in category:
        return random.choice(RELIGION_WELCOME_MESSAGES)
    elif "فلسف" in category or "philosophy" in category:
        return random.choice(PHILOSOPHY_WELCOME_MESSAGES)
    elif "تعارف" in category or "social" in category:
        return random.choice(SOCIAL_WELCOME_MESSAGES)
    elif "عام" in category or "general" in category:
        return random.choice(GENERAL_WELCOME_MESSAGES)
    else:
        return "مرحباً بك في هذا الموضوع!"
